...
inline void request::openfile(std::ifstream& fin,const string& filename = fileaddress)
{
	fin.open(fileaddress.c_str());
}
inline string request::add(const string& temp,string &home_url)
{
	int now = 0;
	int flag  = 0;
